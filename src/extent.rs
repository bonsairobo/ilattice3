...
impl Extent {
    ...
    /// Returns an iterator over the points on the boundary of `self` (those points that are both in
    /// `self` and adjacent to some point not in `self`).
...
mod tests {
    ...
    #[test]
    fn test_extent_does_not_contain_adjacent_points() {
        let extent = Extent::from_min_and_local_supremum([1, 1, 1].into(), [1, 3, 2].into());

        // Each adjacent to one face of extent.
        let adjacent_extents = vec![
    ...
    #[test]
    fn test_extent_bounding_set_of_points() {
        let points: [Point; 5] = [
            (-1, 5, 3).into(),
            (0, 4, 2).into(),
            (8, 0, 1).into(),
